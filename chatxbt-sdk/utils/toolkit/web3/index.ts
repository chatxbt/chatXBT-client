...
export const getPriceFromCoingecko = async (
  ...
  const crypto = token.coinGeckoId,
    fiat = fromCrypto ? to : from;
  ...
  const value = fromCrypto
    ? amount * parseFloat(rateOfCryptoToFiat)
    : amount / parseFloat(rateOfCryptoToFiat);
  return {
    status: true,
    type: "coin-price",
...
export const getCoinMarketChartFromCoinGecko = async (
  coin: string,
  amount: number,
  to: string = "usd"
) => {
...
export const checkIfprotocolExist = (
...
export const findMatchingDex = (
  messageObject: { [x: string]: any },
  dexes: string | any[]
) => {
